...
function trySparkdataboxUrl(imageUrl) {
  ...
  if (imageUrl.includes('newsaddaindia.com/wp-content/uploads/')) {
    ...
    const sparkUrl = imageUrl
      .replace('https://newsaddaindia.com', 'http://sparkdatabox.tk')
      .replace('http://newsaddaindia.com', 'http://sparkdatabox.tk');
    return sparkUrl;
  }
      ...
      if (error.response && error.response.status === 404 && imageUrl.includes('newsaddaindia.com')) {
        ...
        if (sparkUrl) {
          ...
          try {
            ...
            // Silently handle domain resolution failures (ENOTFOUND, ECONNREFUSED, ETIMEDOUT)
...
const categoryMapping = {
  ...
  'breaking-news': 'National', // Alternative format
...
async function importWordPressXML(xmlFilePath) {
  try {
    ...
    console.log(`   Skipped (no title/content): ${stats.skipped}`);
...
// Main execution
async function main() {
  // XML file path - try multiple possible locations
  const possiblePaths = [
    path.resolve(__dirname, '../../newsaddaindia_jan_mar.xml'), // Project root
    'C:\\Users\\aseem.srivastava\\OneDrive - Prodapt Solutions Private Limited\\Documents\\projects\\NewsAddaIndia\\newsaddaindia_jan_mar.xml', // Absolute path
    path.join(process.cwd(), 'newsaddaindia_jan_mar.xml'), // Current working directory
  ];
  ...
  // If not found, try the absolute path from user's input
  if (!xmlFilePath) {
    const absolutePath = 'C:\\Users\\aseem.srivastava\\OneDrive - Prodapt Solutions Private Limited\\Documents\\projects\\NewsAddaIndia\\newsaddaindia_jan_mar.xml';
  ...
  if (!xmlFilePath || !fs.existsSync(xmlFilePath)) {
    ...
    console.log('\nPlease ensure the XML file exists at one of these locations.');
    process.exit(1);
  }
  ...
  // Import the XML file
  await importWordPressXML(xmlFilePath);
