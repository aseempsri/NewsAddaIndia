    ...
    <header class="main-header glass-card border-b border-border/30 transition-all duration-300 w-full bg-background/95 backdrop-blur-sm" [style.top.px]="0" [style.position]="getHeaderPosition()" [style.left]="'0'" [style.right]="'0'" [style.transform]="'translateZ(0)'">
        <div class="container mx-auto px-4 py-3">
          <div class="flex items-center justify-between gap-2.5">
            <!-- Logo -->
            <a [routerLink]="'/'" (click)="onNavLinkClick('/')" aria-label="Social Screen home" class="flex items-center shrink-0 group min-w-0">
              <img
                [src]="logoPath"
                alt=""
                class="block h-10 sm:h-12 md:h-14 w-auto max-w-[min(300px,72vw)] object-contain object-left transition-opacity group-hover:opacity-90"
                loading="eager"
                height="56" />
            </a>
