...
const STORAGE_KEY = 'newsaddaindia_news_date';
...
export class NewsDateFilterService {
  ...
  /** Desktop only — mobile always uses today for news API calls */
  ...
  toApiParam(): string {
    const date = this.isMobileViewport() ? this.startOfDay(new Date()) : this.getSelectedDate();
    return this.toIsoDateStringIST(date);
  }
  ...
  formatTodayDisplay(locale: string): string {
