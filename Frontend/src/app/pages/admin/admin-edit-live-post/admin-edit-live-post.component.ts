                ...
                <form (ngSubmit)="updateNews()" (input)="markFormDirty()" (change)="markFormDirty()" enctype="multipart/form-data" class="space-y-6">
                  ...
                  <div class="space-y-4">
                    ...
                    <div class="mt-2 space-y-2">
                      ...
                      <p class="text-xs text-muted-foreground mt-1 ml-6">Send push notification(s) to subscribers when this post is updated. Select one or both languages.</p>
                    </div>
...
export class AdminEditLivePostComponent implements OnInit, OnDestroy, CanDeactivate {
  ...
  openGalleryPicker() {
    ...
    this.galleryPickerSub = this.imageGalleryPicker.selected$.pipe(
      take(1)
    ).subscribe(paths => {
  ...
  async translateTitleToEnglish() {
    ...
    try {
      ...
      console.error('[AdminEditLivePost] Error translating title:', error);
  ...
  async translateExcerptToEnglish() {
    ...
    try {
      ...
      console.error('[AdminEditLivePost] Error translating excerpt:', error);
  ...
  async translateSummaryToEnglish() {
    ...
    try {
      ...
      console.error('[AdminEditLivePost] Error translating summary:', error);
  ...
  async translateContentToEnglish() {
    ...
    try {
      ...
      console.error('[AdminEditLivePost] Error translating content:', error);
  ...
  async translateTrendingTitleToEnglish() {
    ...
    try {
      ...
      console.error('[AdminEditLivePost] Error translating trending title:', error);
  ...
  updateNews() {
    ...
    // Build existingImagePaths: current from post + gallery selection (paths to keep)
